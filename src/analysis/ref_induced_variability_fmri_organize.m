...
for isub = runSub
    ...
    % Within subject (behavior)
    err_corr = cond_correct(errme, errme, ref, stimulus, 'c', 'c'); 
    for ir = 1:length(refs)
        ind = find(ref==refs(ir) & ~isnan(errme) & abs(errme)<lapse_crit); 
        matMerge_sub.mean(ir,isub) = circ_m(errme(ind)'); 
        matMerge_sub.iqr(ir,isub)  = circ_s(errme(ind)'); %iqr(errme(ind)); 
        
        ind = find(ref==refs(ir) & ~isnan(err_corr) & abs(errme)<lapse_crit); 
        matMerge_sub_refcorr.mean(ir,isub) = circ_m(err_corr(ind)'); 
        matMerge_sub_refcorr.iqr(ir,isub)  = circ_s(err_corr(ind)'); %iqr(err_corr(ind)); 
    end
    
    errme_fmri_corr = nan(size(errme_fmri));
    for iTR = 1:nTR
        errme_fmri_corr(iTR,:) = cond_correct(errme, errme_fmri(iTR,:), ref, stimulus, 'c', 'c'); 
        for ir = 1:length(refs)
            ind = find(ref==refs(ir) & ~isnan(errme) & abs(errme)<lapse_crit); 
            matfMRI_sub.mean(iTR,ir,isub) = circ_m(errme_fmri(iTR,ind)'); 
            matfMRI_sub.iqr(iTR,ir,isub)  = circ_s(errme_fmri(iTR,ind)'); %iqr(errme(ind)); 

            ind = find(ref==refs(ir) & ~isnan(errme) & abs(errme)<lapse_crit & timing==1); 
            matfMRI_sub.mean_e(iTR,ir,isub) = circ_m(errme_fmri(iTR,ind)'); 
            matfMRI_sub.iqr_e(iTR,ir,isub)  = iqr(errme_fmri(iTR,ind)); %circ_s(errme_fmri(iTR,ind)'); 

            ind = find(ref==refs(ir) & ~isnan(errme) & abs(errme)<lapse_crit & timing==2); 
            matfMRI_sub.mean_l(iTR,ir,isub) = circ_m(errme_fmri(iTR,ind)'); 
            matfMRI_sub.iqr_l(iTR,ir,isub)  = iqr(errme_fmri(iTR,ind));  %circ_s(errme_fmri(iTR,ind)');
            
            
            ind = find(ref==refs(ir) & ~isnan(errme) & abs(errme)<lapse_crit); 
            matfMRI_sub_refcorr.mean(iTR,ir,isub) = circ_m(errme_fmri_corr(iTR,ind)'); 
            matfMRI_sub_refcorr.iqr(iTR,ir,isub)  = circ_s(errme_fmri_corr(iTR,ind)'); %iqr(err_corr(ind)); 

            ind = find(ref==refs(ir) & ~isnan(errme) & abs(errme)<lapse_crit & timing==1); 
            matfMRI_sub_refcorr.mean_e(iTR,ir,isub) = circ_m(errme_fmri_corr(iTR,ind)'); 
            matfMRI_sub_refcorr.iqr_e(iTR,ir,isub)  = circ_s(errme_fmri_corr(iTR,ind)'); %iqr(err_corr(ind)); 

            ind = find(ref==refs(ir) & ~isnan(errme) & abs(errme)<lapse_crit & timing==2); 
            matfMRI_sub_refcorr.mean_l(iTR,ir,isub) = circ_m(errme_fmri_corr(iTR,ind)'); 
            matfMRI_sub_refcorr.iqr_l(iTR,ir,isub)  = circ_s(errme_fmri_corr(iTR,ind)'); %iqr(err_corr(ind)); 
        end
    end
end
...
for ir = 1:length(refs)
    ...
    matMerge.mean(ir) = circ_m(err_m(ind)'); 
    matMerge.iqr(ir)  = circ_s(err_m(ind)');%iqr(err_m(ind)); 
end
